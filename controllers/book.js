    ...
    try {
        ...
        if(title && description){
            ...
            if(check){
                ...
                return res.status(201).send({message: 'Book added successfully', book: newBook});
            }
        }
        else{
...
const addBookToBookList = async(req,res) => {
    try {
        ...
        if(!user){
            ...
            if(check){
                return res.status(400).send({message: 'Book already added to book list'});
            }
            else{
...
module.exports = {addBook, searchBook, addBookToBookList};